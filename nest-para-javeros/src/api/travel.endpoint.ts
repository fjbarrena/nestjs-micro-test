...
import { multer, uuidv4, fs, TRAVEL_IMAGE_FOLDER } from 'main';
...
export class TravelEndpoint extends GenericController<Travel> {
    ...
    async imagesFromTravel(
        ...
        @Req() req, @Res() res) {
            ...
            fs.readFile(TRAVEL_IMAGE_FOLDER + file, function (err, content) {
                if (err) {
    ...
    async deleteImageFromTravel(
        ...
        @Req() req, @Res() res) {
            ...
            fs.unlink(TRAVEL_IMAGE_FOLDER + file, function(err, content) {
                if (err) {
                    res.writeHead(400, {'Content-type':'text/html'})
                    console.log(err);
                    res.end('No such image');    
